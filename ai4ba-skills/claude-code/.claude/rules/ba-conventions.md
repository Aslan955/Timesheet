...
* **Câu trả lời CẤP DỰ ÁN** (domain, thuật ngữ gọi người dùng, đối thủ, compliance...) → đọc trước + ghi lại vào `docs/_shared/project-profile.md` theo `.claude/rules/project-profile.md` — no-re-ask áp cross-session và cross-skill qua file này.
...
**Công thức 1-2 câu (~120-250 ký tự), theo thứ tự:**

1) **Dùng khi cần {việc gì} → ra {output gì}** — 1 câu, từ nghiệp vụ. Đây là phần bắt buộc và
   thường là phần DUY NHẤT cần có.
2) *(chỉ khi cú pháp gọi mang thông tin thật)* **cú pháp gọi** — khi có tham số/biến thể đổi hành vi
   mà tên skill không nói lên được, vd `/gap <feature> · --product đối chiếu Feature Map ↔ Roadmap`.
   Viết thẳng cú pháp, **KHÔNG mở đầu bằng "Kích hoạt bằng"** — 4 chữ đó không thêm thông tin nào
   (skill vốn được gọi bằng tên của nó, và `argument-hint` đã khai tham số).
3) *(chỉ khi thật sự dễ nhầm)* **1 vế phân biệt** với skill anh em gần nhất, dạng "làm {X} thì dùng `/{skill-kia}`".
...
* Luật vận hành (vd "KHÔNG tự lấy cwd") — luật phải sống trong **Constraints** của thân SKILL.md,
...
**Kiểm nhanh:** cắt xong tự hỏi *"còn phân biệt được skill này với skill gần nhất không?"* — còn thì
đủ ngắn. Mỗi chi tiết định cắt phải đã tồn tại trong thân SKILL.md hoặc rule liên quan; nếu chỉ nằm ở
description thì **chuyển vào chỗ đúng trước**, rồi mới cắt.‍​​‌‌​‌‌​‌‌‌​​‌​‌‌‌​​​​​​‌​‌‌​‌‌‌‌‌‌​​​​‌​‌‌​‌‌‌‌​‌‌‌‌‌‌‌‌​‌‌​‌​​‌​‌‌‌​​​‌‌‌‌‌‌​​​‌​‌‌‌​​‌‌‌‌‌​​‌​‌‌‌‌‌​‌​​‌‌​​‌​​‌​​​​‌‌​​​​​​‌​‍
