...
export const DependentPage: React.FC = () => {
  ...
  interface DependentFormItem {
    id: string;
    dependentName: string;
    dob: string;
    taxCode: string;
    idCard: string;
    relationship: 'Con' | 'Cha/ Mẹ' | 'Khác';
    isCalculated: boolean;
    startDate: string;
    endDate: string;
  }

  const [formDependents, setFormDependents] = useState<DependentFormItem[]>([]);
  ...
  const handleOpenAddModal = (emp?: Employee) => {
    ...
    if (targetEmp) {
      ...
      setIsCustomEmployee(!employees.some(e => e.staffId === targetEmp.staffId));
    } else {
      setFormEmployeeName('');
  ...
  const handleOpenEditModal = (dep: Dependent) => {
    ...
    setIsCustomEmployee(!employees.some(e => e.staffId === dep.employeeStaffId));
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    for (let i = 0; i < formDependents.length; i++) {
      const dep = formDependents[i];
      if (!dep.dependentName.trim()) {
        setFormError(`Vui lòng nhập họ tên thành viên phụ thuộc thứ ${i + 1}`);
        return;
      }
    }

    if (editingDependent) {
      // Edit mode: save exactly 1 dependent
      const depItem = formDependents[0];
      const savedData: Dependent = {
        ...
        dependentName: depItem.dependentName.trim(),
        dob: depItem.dob,
        taxCode: depItem.taxCode.trim(),
        idCard: depItem.idCard.trim(),
        relationship: depItem.relationship,
        isCalculated: depItem.isCalculated,
      ...
      // Create mode: add list of dependents
      const newDependentsList: Dependent[] = formDependents.map((depItem, idx) => ({
        id: `DP-${Date.now().toString().slice(-4)}-${idx}-${Math.floor(Math.random() * 100)}`,
        employeeName: formEmployeeName.trim(),
        employeeStaffId: formEmployeeStaffId.trim(),
        dependentName: depItem.dependentName.trim(),
        dob: depItem.dob,
      ...
      setDependents(prev => [...newDependentsList, ...prev]);
      
      // Auto-add employee if they don't exist yet in the primary listing
      const empExists = employees.some(emp => emp.staffId === formEmployeeStaffId.trim());
      if (!empExists) {
        const newEmp: Employee = {
          staffId: formEmployeeStaffId.trim(),
          name: formEmployeeName.trim(),
          department: 'Phòng ban mới',
          position: 'Nhân sự ngoài danh sách',
          email: `${formEmployeeStaffId.trim().toLowerCase()}@imisportal.com`,
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select 
                        value={isCustomEmployee ? "custom" : formEmployeeStaffId}
                        onChange={(e) => {
                          ...
                          if (val === "custom") {
                            ...
                            setIsCustomEmployee(false);
                            const emp = employees.find(x => x.staffId === val);
                            if (emp) {
                              setFormEmployeeName(emp.name);
                              setFormEmployeeStaffId(emp.staffId);
                            } else {
                              setFormEmployeeName('');
                              setFormEmployeeStaffId('');
                            }
                        ...
                        className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10 cursor-pointer"
                        required
                      >
                        ...
                        <option value="custom">✍️ Nhập nhân sự mới...</option>
                      </select>
                      
                      {isCustomEmployee && (
                        <div className="mt-2.5">
                          <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Nhập họ tên nhân sự *</label>
                          <input 
                            type="text"
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        disabled={!isCustomEmployee && formEmployeeStaffId !== ''}
                        className={`w-full px-3.5 py-2 border rounded-xl text-xs font-mono font-bold outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10 ${
                          !isCustomEmployee && formEmployeeStaffId !== '' 
                 ...
                 <div className="space-y-6">
                   <div className="flex items-center justify-between">
                     ...
                     {!editingDependent && (
                       <button
                         ...
                         className="px-3 py-1.5 bg-emerald-50 hover:bg-[#0fa57c]/10 text-[#0fa57c] border border-emerald-100/50 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all flex items-center gap-1 cursor-pointer active:scale-95"
                       >
                         <Plus size={12} />
                         Thêm người phụ thuộc
                       </button>
                   ...
                   <div className="space-y-6">
                     {formDependents.map((dep, index) => (
                       <div key={dep.id} className="p-5 border border-slate-200/80 rounded-2xl space-y-4 relative bg-slate-50/20 shadow-xs">
                         
                         {/* Card Header with Counter and Delete Option */}
                         <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                           <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                             Người phụ thuộc #{index + 1}
                           </span>
                           
                           {!editingDependent && formDependents.length > 1 && (
                             <button
                               type="button"
                               onClick={() => {
                                 setFormDependents(prev => prev.filter(item => item.id !== dep.id));
                               }}
                               className="px-2 py-1 text-[10px] text-rose-500 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-lg transition-colors flex items-center gap-1 font-bold cursor-pointer"
                             >
                               <Trash2 size={12} />
                               Xóa người này
                             </button>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                           <div>
                             ...
                             <input 
                               ...
                               onChange={(e) => {
                                 const val = e.target.value;
                                 setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, dependentName: val } : item));
                               }}
                               className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10"
                               required
                             />
                           ...
                           <div>
                             ...
                             <div className="relative">
                               ...
                               <input 
                                 ...
                                 onChange={(e) => {
                                   const val = e.target.value;
                                   setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, dob: val } : item));
                                 }}
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                           <div>
                             ...
                             <input 
                               ...
                               onChange={(e) => {
                                 const val = e.target.value;
                                 setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, taxCode: val } : item));
                               }}
                           ...
                           <div>
                             ...
                             <input 
                               ...
                               onChange={(e) => {
                                 const val = e.target.value;
                                 setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, idCard: val } : item));
                               }}
                               className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-mono font-semibold outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10"
                             />
                           </div>
                             ...
                             <div className="grid grid-cols-3 gap-2">
                               {(['Con', 'Cha/ Mẹ', 'Khác'] as const).map((rel) => (
                                 <button
                                   ...
                                   type="button"
                                   onClick={() => {
                                     setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, relationship: rel } : item));
                                   }}
                           ...
                           <div>
                             ...
                             <label className="flex items-center space-x-3 p-2 bg-slate-50 border border-slate-100 rounded-xl cursor-pointer hover:bg-slate-100/80 transition-colors">
                               <div className="relative">
                                 <input 
                                   type="checkbox"
                                   checked={dep.isCalculated}
                                   onChange={(e) => {
                                     const val = e.target.checked;
                                     setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, isCalculated: val } : item));
                                   }}
                                   className="sr-only"
                                 />
                                 <div className={`w-10 h-6 rounded-full transition-all duration-300 ${dep.isCalculated ? 'bg-[#0fa57c]' : 'bg-slate-300'}`} />
                                 <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform duration-300 ${dep.isCalculated ? 'translate-x-4' : 'translate-x-0'}`} />
                               </div>
                           ...
                           <div>
                             ...
                             <div className="relative">
                               ...
                               <input 
                                 ...
                                 onChange={(e) => {
                                   const val = e.target.value;
                                   setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, startDate: val } : item));
                                 }}
                                 className="w-full pl-3.5 pr-10 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10"
                               />
                             </div>
                           ...
                           <div>
                             ...
                             <div className="relative">
                               ...
                               <input 
                                 ...
                                 onChange={(e) => {
                                   const val = e.target.value;
                                   setFormDependents(prev => prev.map(item => item.id === dep.id ? { ...item, endDate: val } : item));
                                 }}
                                 className="w-full pl-3.5 pr-10 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium outline-none focus:border-[#0fa57c] focus:ring-2 focus:ring-[#0fa57c]/10"
                               />
                             </div>
                   ...
                   {/* Dynamic Add button in the bottom area too for convenience */}
                   {!editingDependent && (
                     <button
                       type="button"
                       onClick={() => {
                       ...
                       className="w-full py-3 border-2 border-dashed border-slate-200 hover:border-[#0fa57c] hover:bg-[#0fa57c]/5 text-[#0fa57c] rounded-2xl text-[11px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer"
                     >
                       <Plus size={14} />
                       Cộng thêm người phụ thuộc mới
                     </button>
                   )}
                 </div>
