...
Skill KẾT THÚC turn ở đây. __TUYỆT ĐỐI không tự đi tiếp sang apply.__ Đợi user reply `apply`/`cancel`/câu hỏi. Nếu user đã nói "chỉ phân tích thôi" ngay từ đầu: chỉ in report path rồi dừng, KHÔNG kèm gợi ý `apply`.
...
KHÔNG được skip bước này. Phải in __mọi file đã edit + xác nhận từng file có dòng changelog.md tương ứng__ để user thấy rõ.
