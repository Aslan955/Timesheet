...
export const LeaveBalancesPage: React.FC = () => {
  const [activeMainTab, setActiveMainTab] = useState<'personal' | 'management' | 'report'>('personal');
  ...
  return (
    <div className="bg-transparent min-h-full">
      ...
      <div className="bg-white border-b border-gray-200 px-6 pt-2 sticky top-0 z-20 shadow-sm font-sans font-medium tracking-tight">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center text-xs text-gray-400 font-medium">
             ...
             <span className="text-gray-900 font-bold">Phép cá nhân & Báo cáo</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-8">
          ...
          <button 
            ...
            className={`pb-3 text-sm font-bold transition-all relative cursor-pointer ${
              activeMainTab === 'report' ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            Báo cáo ngày nghỉ phép
            <span className="ml-2 px-1.5 py-0.5 bg-emerald-100 text-[#0fa57c] text-[10px] rounded-full font-black">Mới</span>
            {activeMainTab === 'report' && <motion.div layoutId="mainTabIndicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600" />}
          </button>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        {activeMainTab === 'report' && <LeaveReportPage />}
      </div>
