...
Ngoài ba nguồn trên, còn một loại thông tin không thuộc riêng tính năng nào mà thuộc về **cả dự án**: sản phẩm này ở lĩnh vực gì, người dùng cuối được gọi là gì (học viên / khách hàng / tài xế / bệnh nhân...), có quy định pháp lý nào phải tuân (bảo mật dữ liệu cá nhân chẳng hạn — thứ hay chạm tới khi bàn phần rủi ro).

Những câu đó được lưu ở một file dùng chung tên là **hồ sơ dự án** (`docs/_shared/project-profile.md`). Cơ chế gồm bốn bước: `/brainstorm` **đọc hồ sơ trước** khi hỏi → thiếu phần nào thì **chỉ hỏi phần đó** → trả lời xong nó **xin phép ghi vào hồ sơ** (bạn duyệt trước, từ chối cũng được) → **mọi lệnh sau** (`/urd`, `/brd`, `/srs`...) đọc chung hồ sơ nên không hỏi lại bạn nữa.

Nói cách khác, nguyên tắc "không hỏi lại" không chỉ áp trong một buổi làm việc, mà kéo dài **xuyên suốt cả dự án và qua mọi lệnh**. Muốn xem/sửa hồ sơ chủ động thì dùng `/update-overview profile`.
